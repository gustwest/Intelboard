...
export default function EditEventScreen() {
  ...
  useEffect(() => {
    // Fetch courts and the event details
    Promise.all([
      api.get<Court[]>('/api/mobile/courts').catch(() => []),
      api.get<any>(`/api/mobile/events/${id}`).catch(() => null)
    ]).then(([courtsData, eventData]) => {
      ...
      if (eventData) {
        ...
        setMaxPlayers(eventData.maxPlayers || 8);
        setSkillLevel(eventData.skillLevel || '');
        if (eventData.court && Array.isArray(courtsData)) {
          const court = courtsData.find(c => c.id === eventData.court.id);
          if (court) setSelectedCourt(court);
        }
      } else {
        Alert.alert('Fel', 'Kunde inte ladda eventdetaljer');
        router.back();
      }
      setLoading(false);
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      let startsAt;
      if (date && time) {
        startsAt = new Date(`${date}T${time}:00Z`).toISOString(); // Simple approximation
      }
      ...
      const result = await api.patch<{ success: boolean; event: any }>(`/api/mobile/events/${id}`, eventData);
      if (result?.success) {
        Alert.alert('Sparat!', 'Ändringarna har sparats', [{ text: 'OK', onPress: () => router.back() }]);
      } else {
        router.back();
      }
    } catch (err: any) {
      console.warn('Failed to edit event:', err);
      Alert.alert('Fel', err.message || 'Kunde inte redigera evenemang');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Redigera speltid</Text>
        <View style={{ width: 32 }} />
      </View>
          ...
          <LinearGradient colors={['#ea580c', '#db2777']} start={{x:0,y:0}} end={{x:1,y:0}} style={s.submitGradient}>
            {submitting ? (
              ...
              <>
                ...
                <Text style={s.submitText}>Spara ändringar</Text>
              </>
            )}
          </LinearGradient>
        </TouchableOpacity>
