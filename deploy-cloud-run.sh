...
IMAGE_NAME="intelboard-app"
SERVICE_NAME="intelboard-app"
...
"$GCLOUD_BIN" artifacts repositories create "$REPO_NAME" \
    ...
    --location="$REGION" \
    --description="Docker repository for Intelboard" \
    --project="$PROJECT_ID" || echo "Repository likely already exists, proceeding..."

# 3. Build & Submit Image (using Cloud Build)
echo "Building and Submitting Container Image (this takes a while)..."
# We need to make sure we are not sending huge node_modules context
