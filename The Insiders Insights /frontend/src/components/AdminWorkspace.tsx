...
// ============================================================
// MAIN WORKSPACE — shared by The Insiders (/admin) and
// Insider Graph (/insider-graph/admin). The `product` prop scopes
...
// KANBAN TAB
// ============================================================
function KanbanTab({ product }: { product: string }) {
  ...
  const fetchIssues = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/api/issues?product=${encodeURIComponent(product)}`);
  ...
  return (
    <>
      ...
      {/* New Issue Modal */}
      {showNew && <NewIssueModal product={product} onClose={() => setShowNew(false)} onCreated={(issue) => { setIssues(prev => [issue, ...prev]); setShowNew(false); }} />}
...
// NEW ISSUE MODAL
// ============================================================
function NewIssueModal({ product, onClose, onCreated }: { product: string; onClose: () => void; onCreated: (issue: KanbanIssue) => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/issues`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, description, product }),
      });
...
// FILES TAB
// ============================================================
function FilesTab({ product }: { product: string }) {
  ...
  const fetchFiles = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/api/files?product=${encodeURIComponent(product)}`);
...
function AgentTab({ product }: { product: string }) {
  const modelKey = `admin-agent-model-${product}`;
