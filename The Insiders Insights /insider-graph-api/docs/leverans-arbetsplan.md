...
> Schemaläggning KLAR: fan-out-jobb `jobs/customer_report_email_all.py` + registrerat i cloudbuild.yaml JOB-loopen + bootstrap.sh (`create_or_update_job` + `schedule_job customer-report-email-monthly "30 7 1 * *"` — 1:a 07:30, efter monthly-report-all 07:00). Cloud Run Job + scheduler skapade på staging. **OBS:** jobb-env saknar SendGrid-nycklar (som alla jobb idag) → faktisk sändning self-no-op:ar tills SendGrid konfigureras — medvetet (vill inte mejla riktiga kunder från staging).
